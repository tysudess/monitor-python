...
def test_video_editor_page_shutdown_stops_embedded_editor(tmp_path: Path):
    _app()
    page = VideoEditorPage(tmp_path)
    page.show()
    page.open_editor()

    # O editor agora faz parte da página principal e não pode voltar a ser uma
    # janela top-level separada.
    assert page.editor.parentWidget() is page
    assert not page.editor.isWindow()
