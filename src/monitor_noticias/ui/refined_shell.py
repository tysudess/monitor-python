...
class RadarHeader(QFrame):
    """Cabeçalho compartilhado no padrão visual claro do Monitor."""
...
class TechFooter(QFrame):
    def __init__(self) -> None:
        ...
        self.status.setStyleSheet("color:#4F6E98;font-size:9px;")
        row.addWidget(self.status)

    def set_busy(self, busy: bool) -> None:
        self.status.setText("●  Busca em andamento" if busy else "●  Sistema operacional")
        self.status.setStyleSheet(
            "color:#C78900;font-size:9px;" if busy else "color:#078C63;font-size:9px;"
        )
